...
start_time_str = datetime.strptime('03.11.2020 13:00:00', '%d.%m.%Y %H:%M:%S')
start_time_unix = start_time_str.timestamp() * 1000

end_time_str = datetime.strptime('03.11.2020 15:00:00', '%d.%m.%Y %H:%M:%S')
...
fig = px.scatter(x=time_values, y=frequency_values, labels={'x': 'Tempo', 'y': 'Frequência [Hz]'})
fig.show()

fig = px.scatter(x=[1, 2, 3], y=[1, 2, 3], labels={'x': 'Hey', 'y': 'Hi [Hz]'})
fig.show()
